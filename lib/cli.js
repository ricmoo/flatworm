        ...
        try {
            ...
            if (opts.flags.version) {
                ...
                if (!opts.flags.skipeval) {
                    const script = new script_1.Script(config.codeRoot ? path_1.resolve(src, config.codeRoot) : src, config.codeContextify || null);
                ...
                renderers.forEach((renderer) => {
                    ...
                    files.forEach((file) => {
                        const filename = path_1.resolve(dst, file.filename);
                        fs_1.default.mkdirSync(path_1.dirname(filename), { recursive: true });
