...
var _IndexGroup_entries;
import fs from "fs";
import { dirname, join, resolve } from "path";
import { fileURLToPath } from 'url';
import { Config } from "./config2.js";
import { BodyContent, CodeContent, Document, Section } from "./document2.js";
import { ElementNode, LinkNode, ListNode, TextNode, parseMarkdown } from "./markdown.js";
import { API, ClassExport, ConstExport, PropertyExport, Export, FunctionExport, ObjectExport, TypeExport, TypeBasic, TypeFunction, TypeIdentifier, TypeGroup, TypeLiteral, TypeMapping, TypeTodo, TypeWrapped, ApiSection } from "./jsdocs.js";
...
export class IndexGroup {
    constructor(header) {
        _IndexGroup_entries.set(this, void 0);
        this.header = header;
        __classPrivateFieldSet(this, _IndexGroup_entries, [], "f");
    }
    addEntry(entry) {
        __classPrivateFieldGet(this, _IndexGroup_entries, "f").push(entry);
    }
    get entries() {
        const entries = __classPrivateFieldGet(this, _IndexGroup_entries, "f").slice();
        entries.sort((a, b) => (nameCompare(a.sort, b.sort)));
        return entries;
    }
}
_IndexGroup_entries = new WeakMap();
function splitCamelFirstWord(v) {
    // Single letter or non-letter; e.g. "P" or "_foo"
    if (v.length < 2 || !v.match(/^[a-z]/i)) {
        return [v];
    }
...
export function getIndex(api) {
    ...
    for (const obj of api.objs) {
        ...
        if (obj instanceof ClassExport) {
            ...
            const supers = [];
            const superProps = new Set();
            const superStaticProps = new Set();
            for (const s of api.getSupers(obj.name)) {
                supers.push(s.name);
                for (const n of s.methods.keys()) {
                    superProps.add(n);
                }
                for (const n of s.properties.keys()) {
                    superProps.add(n);
                }
                if (s instanceof ClassExport) {
                    for (const n of s.staticMethods.keys()) {
                        superStaticProps.add(n);
                    }
                }
            }
            const classObjs = [
...
export function generateIndex(api) {
...
;
function prepareLocalToc(path, toc) {
...
function addHeader(api, config, output, section, links, toc) {
...
function addSection(output, links, section) {
...
function addSectionInfo(output, links, section) {
...
function addFooter(config, output, previous, next, genDate) {
...
function addExample(output, script) {
...
function addReturnsExport(output, links, obj) {
...
function addObjectExport(api, output, _links, obj) {
    const name = obj.name;
    const toc = { link: `#${name}`, title: name, style: "code" };
...
function addTypeExport(output, links, obj) {
    const srcLink = links.get(`src:${obj.id}`);
...
function addExports(api, output, links, objs) {
    const toc = [];
    // Show all types
    const types = objs.filter((o) => (o instanceof TypeExport));
    if (types.length) {
    ...
    // Show all types
    const consts = objs.filter((o) => (o instanceof ConstExport));
    if (consts.length) {
    ...
    // Show all functions
    const funcs = objs.filter((o) => (o instanceof FunctionExport));
    if (funcs.length) {
...
export function generate(api, doc, config) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        */
        ...
        for (const section of doc.sections) {
            ...
            if (section.anchor) {
                if (links.has(section.anchor)) {
                    throw new Error(`duplicate anchor: ${section.anchor}`);
                }
                links.set(section.anchor, {
                    link: filename,
                    style: "normal",
            ...
            for (const subsection of section.subsections) {
                ...
                if (links.has(subsection.anchor)) {
                    throw new Error(`duplicate anchor: ${subsection.anchor}`);
                }
        ...
        const pris = mainToc.reduce((accum, { entry, section }) => {
        ...
        mainToc.sort((a, b) => {
            ...
            while (compsA.length && compsA[0] === "") {
                compsA.shift();
            }
            while (compsA[compsA.length - 1] === "") {
                compsA.pop();
            }
            const compsB = b.entry.link.split("#")[0].split("/");
            while (compsB.length && compsB[0] === "") {
                compsB.shift();
            }
            while (compsB[compsB.length - 1] === "") {
                compsB.pop();
            }
            let prefix = "/";
        ...
        const getTs = (path) => __awaiter(this, void 0, void 0, function* () {
            ...
            if (ts == null) {
                ts = yield config.getTimestamp(path);
                if (ts == null) {
                    ts = -1;
                }
                tsCache.set(path, ts);
        ...
        for (let i = 0; i < mainToc.length; i++) {
            ...
            addFooter(config, output, previousEntry, nextEntry, getTimestamp(yield getGenDate(section.dependencies)));
